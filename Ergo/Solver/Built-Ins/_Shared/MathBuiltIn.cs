...
public abstract class MathBuiltIn : BuiltIn
{
    ...
    public dynamic Evaluate(ErgoSolver solver, ITerm t, InterpreterScope s)
    {
        ...
        return c.Functor switch
        {
            ...
            => Evaluate(solver, c.Arguments[0], s)
            ,
            var f when c.Arguments.Length == 2 && WellKnown.Functors.Gt.Contains(f)
            => Evaluate(solver, c.Arguments[0], s) > Evaluate(solver, c.Arguments[1], s)
            ,
            ...
            => (int)(Evaluate(solver, c.Arguments[0], s) / Evaluate(solver, c.Arguments[1], s))
            ,
            var f when c.Arguments.Length == 2 && WellKnown.Functors.Power.Contains(f)
            => Math.Pow(Evaluate(solver, c.Arguments[0], s), Evaluate(solver, c.Arguments[1], s))
            ,
            var f when c.Arguments.Length == 1 && WellKnown.Functors.SquareRoot.Contains(f)
            => Math.Sqrt(Evaluate(solver, c.Arguments[0], s))
            ,
