...
namespace Ergo.Solver.BuiltIns
{
    public sealed class FindAll : BuiltIn
    {
        ...
        public override IEnumerable<Evaluation> Apply(ErgoSolver solver, SolverScope scope, ITerm[] args)
        {
            ...
            {
                if(args[2].IsGround && args[2] == Literals.EmptyList)
                {
                    yield return new Evaluation(Literals.True);
                }
                else if(!args[2].IsGround)
                {
                    yield return new Evaluation(Literals.True, new Substitution(args[2], Literals.EmptyList));
                }
                else
                {
            ...
            else
            {
                ...
                {
                    yield return new Evaluation(Literals.True, new Substitution(args[2], list.Root));
                }
                else
                {
                    yield return new Evaluation(Literals.False);
